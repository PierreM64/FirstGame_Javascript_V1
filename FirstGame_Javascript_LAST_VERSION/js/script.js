...
function startGame(assetsLoaded) {
   ...
   if (buttonMusic==1) {assets.musicStart.play()}
...
function updateJeu() {
    ...
    if (niveauFini()) {
        scoreNiveau(niveau);
        //changement de musiq quand le jeu se termine
        //assets.musicStart.stop();
        levelMusicStop();
        if (buttnMusic===1) {assets.musicNextLevel.play()}
        titleMusic="musicNextLevel";
...
function niveauSuivant() {
    ...
    //-->remplacement de la fonction stop() par pause() pour que la reprise
        //de la music dans un niveau suivant soit plus agrèable et moins genante
    assets.musicNextLevel.pause();
    levelMusic();
